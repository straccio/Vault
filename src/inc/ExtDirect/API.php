...
class ExtDirect_API {
                ...
                foreach($rMethods as $rMethod) {
                    if(
                        ...
                        if($isRemote) {
                            ...
                            if(!!preg_match('/' . '@acl' . ' ?([\w\.]*,?\ ?)?/', $doc, $matches)) {
                            	$method['acl'] = $matches[1];
                            }else{
                            	unset($method['acl']);
                            }                            
    ...
    private function _print($api) {
        ...
        echo ' = ';
        foreach ($api['actions'] as $k=>$c){
        	$this->_tmpClass=$k;
        	$api['actions'][$k]=array_values(array_filter(&$api['actions'][$k], array($this,'arrayFilterACL')));
        }
        echo json_encode($api);
        echo ';';
    }
    
    function arrayFilterACL(&$v,$k){
    	global
    		$currentUser;
    	if($v['acl']){
    		if($currentUser['directAuths'][$this->_tmpClass . '.' . $v['acl']]){
    			unset($v['acl']);
    			return true;
    		}else{
    			return false;
