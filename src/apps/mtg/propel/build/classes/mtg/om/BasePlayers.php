...
abstract class BasePlayers extends BaseObject  implements Persistent
{
	...
	public function setUsername($v)
	{
		...
		if ($this->username !== $v) {
			$this->username = $v;
			$this->modifiedColumns[] = PlayersPeer::USERNAME;
		}
	...
	public function setPassword($v)
	{
		...
		if ($this->password !== $v) {
			$this->password = $v;
			$this->modifiedColumns[] = PlayersPeer::PASSWORD;
		}
	...
	public function setExp($v)
	{
		...
		if ($this->exp !== $v || $this->isNew()) {
			$this->exp = $v;
			$this->modifiedColumns[] = PlayersPeer::EXP;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->exp = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = PlayersPeer::NUM_COLUMNS - PlayersPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(PlayersPeer::ID) ) {
					...
					$affectedRows = PlayersPeer::doUpdate($this, $con);
				}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setExp($arr[$keys[4]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PlayersPeer::USERNAME)) $criteria->add(PlayersPeer::USERNAME, $this->username);
		if ($this->isColumnModified(PlayersPeer::PASSWORD)) $criteria->add(PlayersPeer::PASSWORD, $this->password);
		if ($this->isColumnModified(PlayersPeer::EMAIL)) $criteria->add(PlayersPeer::EMAIL, $this->email);
		if ($this->isColumnModified(PlayersPeer::EXP)) $criteria->add(PlayersPeer::EXP, $this->exp);
